        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-2 text-gray-900" style="@media (max-width: 400px) {padding: 0.5rem;}">
                    ...
                    <form action="<?php echo e(route('invoices.index')); ?>" method="GET" class="mb-2 p-2 rounded-md shadow-sm bg-white">
                    ...
                    <div class="mb-2 text-right">
                        <a href="<?php echo e(route('invoices.create')); ?>" class="inline-flex items-center pt-2 pb-1.5 pr-2 pl-1 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                            ＋新規
                        </a>
                    </div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 text-xs text-gray-500">
                                ...
                                <tr>
                                    <td class="px-2 py-1 whitespace-nowrap text-center">
                                        <a href="<?php echo e(route('invoices.show', $invoice)); ?>" class="text-blue-600 hover:text-fuchsia-600 hover:underline">
                                            <?php echo e($invoice->invoice_number); ?>

                                        </a>
                                    </td>
                                    <td class="px-2 py-1">
                                        <?php echo e($invoice->project->name ?? 'N/A'); ?>

                                        <?php if($invoice->project): ?>
                                        <a href="<?php echo e(route('projects.show', $invoice->project)); ?>" class="text-blue-600 hover:text-fuchsia-600 ml-1 inline-block align-middle" title="イベント詳細へ">
                                    ...
                                    <td class="px-2 py-1 whitespace-nowrap text-center">
                                        <?php echo e($invoice->client->abbreviation ?? 'N/A'); ?>
                                    ...
                                    <td class="p-1 whitespace-nowrap text-center">
                                        <?php echo e($invoice->delivery_date ? \Carbon\Carbon::parse($invoice->delivery_date)->format('Y/m/d') : '未設定'); ?>　<?php echo e($invoice->delivery_location ?? '未設定'); ?>

                                    </td>
                                    <td class="px-2 py-1 whitespace-nowrap text-center">
                                        <?php echo e($invoice->payment_terms ?? '未設定'); ?>

                                    </td>
                                    <td class="px-2 py-1 sm:pr-3 whitespace-nowrap text-right">
                                        ¥<span class="text-sm"><?php echo e(number_format($invoice->total_amount)); ?></span>
                                    </td>
                                    <td class="px-2 py-1 whitespace-nowrap text-center">
                                        <div class="inline-flex items-center justify-center gap-1 text-xs">
                                            <button type="button" 
                                                    class="status-button px-2 py-1 rounded-full cursor-pointer whitespace-nowrap
                                                        <?php echo e($invoice->status === '作成済み' ? 'bg-gray-200 text-gray-800' : ''); ?>

                                                        <?php echo e($invoice->status === '発行済み' ? 'bg-blue-200 text-blue-800' : ''); ?>

                                                        <?php echo e($invoice->status === '送信済み' ? 'bg-green-200 text-green-800' : ''); ?>"
                                                    data-invoice-id="<?php echo e($invoice->id); ?>"
                                                    data-current-status="<?php echo e($invoice->status); ?>"
                                                    <?php if($invoice->status === '送信済み'): ?> disabled <?php endif; ?>>
                                    ...
                                    <td class="px-2 py-1 whitespace-nowrap text-center">
                                        <div class="flex items-center justify-center space-x-1">
                                            <a href="<?php echo e(route('invoices.show', $invoice)); ?>" class="text-blue-600 hover:text-blue-400" title="詳細">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-box-arrow-right" viewBox="0 0 16 16">
                                                    <path fill-rule="evenodd" d="M10 12.5a.5.5 0 0 1-.5.5h-8a.5.5 0 0 1-.5-.5v-9a.5.5 0 0 1 .5-.5h8a.5.5 0 0 1 .5.5v2a.5.5 0 0 0 1 0v-2A1.5 1.5 0 0 0 9.5 2h-8A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h8a1.5 1.5 0 0 0 1.5-1.5v-2a.5.5 0 0 0-1 0z" />
                                                    <path fill-rule="evenodd" d="M15.854 8.354a.5.5 0 0 0 0-.708l-3-3a.5.5 0 0 0-.708.708L14.293 7.5H5.5a.5.5 0 0 0 0 1h8.793l-2.147 2.146a.5.5 0 0 0 .708.708z" />
                                                </svg>
                                            </a>
                                            <a href="<?php echo e(route('invoices.edit', $invoice)); ?>" class="text-emerald-600 hover:text-emerald-400" title="編集">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pen" viewBox="0 0 16 16">
                                                    <path d="m13.498.795.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001m-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708z" />
                                                </svg>
                                            </a>
                                            <form action="<?php echo e(route('invoices.destroy', $invoice)); ?>" method="POST" class="inline-block" onsubmit="return confirm('この請求書を削除します。よろしいですか？');">
                                                <?php echo csrf_field(); ?>
...
<?php endif; ?><?php /**PATH C:\xampp\htdocs\project-manager\resources\views/invoices/index.blade.php ENDPATH**/ ?>