        ...
        <div class="max-w-cont text-gray-700 pb-20">
            ...
            <div class="flex justify-end mb-3 space-x-4">
                ...
                class="bg-slate-400 hover:bg-slate-300 text-white font-bold text-xs px-4 py-2 rounded-md">
                    log</a>
                <a href="{{ route('quotes.edit', $invoice) }}"
                ...
                <a href="{{ route('quotes.downloadPdfMpdf', $invoice) }}" class="bg-red-600 hover:bg-red-400 text-white font-bold text-xs px-4 py-2 rounded-md">
                    PDF出力
                </a>
            </div>
            <div class="bg-card">
                ...
                <table style="width: 100%; border-collapse: collapse; padding-left: 0.75rem;">
                    <tr>
                        <td style="width: 60%; font-size: 1.2rem; font-weight: 500;">
                            {{ $invoice->client->name ?? 'N/A' }} 御中
                        </td>
                        <td style="width: 40%; font-size: 0.875rem; text-align: right;" class="p-xy">
                            <p style="margin-bottom: 0.25rem;">請求番号：{{ $invoice->quote_number }}</p>
                            <p>発行日：{{ \Carbon\Carbon::parse($invoice->issue_date)->format('Y年m月d日') }}</p>
                        </td>
                    </tr>
                </table>
                            ...
                            <table style="width: 100%; border-collapse: collapse;">
                                <tbody>
                                    <tr style="border: 4px solid #fff;">
                                        ...
                                        <td style="text-align: left; width: 75%; padding: 0.375rem 1rem;">{{ $invoice->subject }}</td>
                                    </tr>
                                    <tr style="border: 4px solid #fff;">
                                        <th class="p-xy" style="text-align: center; background: #f0fdf4; padding: 0.375rem">納入予定日</th>
                                        <td style="text-align: left; width: 75%; padding: 0.375rem 1rem;">{{ $invoice->delivery_date ? \Carbon\Carbon::parse($invoice->delivery_date)->format('Y年m月d日') : '未設定' }}</td>
                                    </tr>
                                    <tr style="border: 4px solid #fff;">
                                        <th class="p-xy" style="text-align: center; background: #f0fdf4; padding: 0.375rem">有効期限</th>
                                        <td style="text-align: left; width: 75%; padding: 0.375rem 1rem;">{{ $invoice->expiry_date }}</td>
                                    </tr>
                                    <tr style="border: 4px solid #fff;">
                                        <th class="p-xy" style="text-align: center; background: #f0fdf4; padding: 0.375rem">納入場所</th>
                                        <td style="text-align: left; width: 75%; padding: 0.375rem 1rem;">{{ $invoice->delivery_location ?: '未設定' }}</td>
                                    </tr>
                                    <tr style="border: 4px solid #fff;">
                                        <th class="" style="text-align: center; background: #f0fdf4; padding: 0.375rem">お支払条件</th>
                                        <td style="text-align: left; width: 75%; padding: 0.375rem 1rem;">{{ $invoice->payment_terms ?: '未設定' }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </td>
                    ...
                    <table class="items-table">
                        ...
                        <tfoot>
                            ...
                            <tr>
                                ...
                                <td class="ftr-cell ftr-grnd-total-txt val-bg-w">¥{{ number_format($invoice->total_amount) }}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
                ...
                <div class="notes-sec">
                    ...
                    <p class="notes-cont">{{ $invoice->notes ?: '-' }}</p>
                </div>
                ...
                <div class="flex justify-end mt-4 space-x-4">
                    ...
                    <a href="{{ url('/quotes/' . $invoice->id . '/pdf-mpdf') }}"
