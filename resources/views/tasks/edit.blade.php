<x-app-layout>
            ...
            <form action="{{ route('tasks.update', $task) }}" method="POST">
                ...
                <div class="grid grid-cols-2 gap-4 mb-2">
                    ...
                    <div>
                        ...
                        <select name="priority" class="block py-1.5 w-full border-gray-300 rounded-md focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm">
                            <option value="高" {{ old('priority', $task->priority) == '高' ? 'selected' : '' }}>高</option>
                            <option value="中" {{ old('priority', $task->priority) == '中' ? 'selected' : '' }}>中</option>
                            <option value="低" {{ old('priority', $task->priority) == '低' ? 'selected' : '' }}>低</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-4 gap-4 mb-2">
                    <div>
                        ...
                        <input type="text" value="{{ $task->creator->name ?? Auth::user()->name }}" disabled class="block py-1.5 w-full border-gray-300 rounded-md focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm">
                    </div>

                    <!-- 依頼日 -->
                    <div class="relative">
                        <label class="block font-medium text-sm text-gray-700">依頼日</label>
                        <input type="text" name="start_date" id="start_date" value="{{ old('start_date', $task->start_date ?? \Carbon\Carbon::today()->format('Y-m-d')) }}" class="block py-1.5 w-full border-gray-300 rounded-md focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm" placeholder="yyyy-mm-dd">
                    ...
                    <div class="relative">
                        ...
                        <input type="time" name="start_time" value="{{ old('start_time', $task->start_time) }}" class="block py-1.5 w-full border-gray-300 rounded-md focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm pl-8">
                ...
                <div>
                    ...
                    <div id="url-fields" class="space-y-2">
                        <!-- 既存のURLを表示 -->
                        @foreach($task->urls as $index => $url)
                            <div class="url-field bg-gray-50 p-2 rounded-md border border-gray-200">
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-2">
                                    <div>
                                        <input type="text" name="urls[{{ $index }}][title]" value="{{ old("urls.$index.title", $url->title) }}" placeholder="タイトル" 
                                    ...
                                    <div>
                                        <input type="text" name="urls[{{ $index }}][memo]" value="{{ old("urls.$index.memo", $url->memo) }}" placeholder="メモ" 
                                            class="block py-1.5 w-full border-gray-300 rounded-md focus:border-indigo-400 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm">
                                    </div>
                                </div>
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <input type="url" name="urls[{{ $index }}][url]" value="{{ old("urls.$index.url", $url->url) }}" placeholder="URL" 
                ...
                <div class="grid grid-cols-2 gap-4 mb-2">
                    <div class="relative">
                        ...
                        <input type="text" name="plans_date" id="plans_date" value="{{ old('plans_date', $task->plans_date ?? \Carbon\Carbon::today()->format('Y-m-d')) }}" class="block py-1.5 w-full border-gray-300 rounded-md focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm" placeholder="yyyy-mm-dd">
                        <div class="absolute inset-y-0 right-0 flex items-center pr-2 mt-5 pointer-events-none">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10m-10 4h10m-6 4h6M5 21h14a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2z"/>
                            </svg>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="due_date" id="due_date" value="{{ old('due_date', $task->due_date ?? \Carbon\Carbon::today()->format('Y-m-d')) }}" class="block py-1.5 w-full border-gray-300 rounded-md focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm" placeholder="yyyy-mm-dd">
                        <div class="absolute inset-y-0 right-0 flex items-center pr-2 mt-5 pointer-events-none">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10m-10 4h10m-6 4h6M5 21h14a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2z"/>
                            </svg>
    ...
    <script>
        ...
        flatpickr("#start_date", {
            dateFormat: "Y-m-d",
            allowInput: true,
            defaultDate: "{{ old('start_date', $task->start_date ?? \Carbon\Carbon::today()->format('Y-m-d')) }}"
        });

        flatpickr("#plans_date", {
            dateFormat: "Y-m-d",
            allowInput: true,
            defaultDate: "{{ old('plans_date', $task->plans_date ?? \Carbon\Carbon::today()->format('Y-m-d')) }}"
        });

        flatpickr("#due_date", {
            dateFormat: "Y-m-d",
            allowInput: true,
            defaultDate: "{{ old('due_date', $task->due_date ?? \Carbon\Carbon::today()->format('Y-m-d')) }}"
        });
    </script>

    <!-- JavaScript（createと完全に同じ） -->
    <script>
        let urlIndex = {{ $task->urls->count() }}; // 既存の数からスタート（0件なら0）
