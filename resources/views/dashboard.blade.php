<x-app-layout>
    ...
    <div class="py-2">
        ...
        <tr>
            <td class="px-2 py-1 whitespace-nowrap">{{ $record->timestamp->format('y/m/d H:i') }}</td>
            <td class="px-2 py-1 whitespace-nowrap">
                ...
                @elseif($record->type === 'break_start')
                    <span class="px-2 py-1 bg-yellow-100 text-yellow-800">中抜け</span>
                @elseif($record->type === 'break_end')
                    <span class="px-2 py-1 bg-blue-100 text-blue-800">戻り</span>
                @elseif($record->type === 'business_trip')
                    <span class="px-2 py-1 bg-purple-100 text-purple-800">出張</span>
                @endif
            </td>
            <td class="px-2 py-1 whitespace-nowrap text-gray-500">
                @if($record->type === 'business_trip')
                    出張先（{{ $record->latitude ? round($record->latitude, 4) . ', ' . round($record->longitude, 4) : '不明' }}）
                @else
                    {{ $record->location ? $record->location->name : '範囲外' }}
                @endif
            </td>
            <td class="px-2 py-1 whitespace-nowrap text-gray-500">
                @if($record->type === 'business_trip')
                    - （出張）
                @else
                    {{ $record->distance ?? '-' }} <span class="text-slate-400"> m</span>
                @endif
            </td>
            <td class="px-2 py-1 whitespace-nowrap">
                @if($record->is_valid)
