                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700 border border-gray-200">
                                    @if ($project->quotes_sum_total_amount > 0 && isset($latestQuote))
                                        <span class="flex items-center">
                                            ...
                                            {{-- PDFダウンロードリンクアイコン --}}
                                            @if ($latestQuote->pdf_path) {{-- pdf_pathが存在する場合のみアイコンを表示 --}}
                                                <a href="{{ route('quotes.downloadPdf', $latestQuote->id) }}" target="_blank" class="ml-2 text-red-600 hover:text-red-800" title="PDFをダウンロード">
