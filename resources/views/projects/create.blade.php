<x-app-layout>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('projects.store') }}">
                        ...
                        <div class="col-span-1" x-data="{ open: false, selectedColor: {{ old('color', $colors->first()->id) }}, selectedHex: '{{ optional($colors->firstWhere('id', (int) old('color', $colors->first()->id)))->hex_code ?? '#3B82F6' }}' }" x-cloak>
                            <x-input-label>カラー</x-input-label>
                            <div class="relative">
                                <button type="button"
                                    @click="open = !open"
                                    class="w-full border border-gray-300 rounded-md px-3 py-1.5 mt-1 flex items-center justify-between focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm">
                    ...
                    <div class="grid grid-cols-12 gap-4 mb-4" x-data="userChips({{ json_encode($users) }}, {{ json_encode(old('users', $projectUsers ?? [])) }})" x-init="init()" x-cloak>
                        ...
                        <div class="col-span-8 border rounded-md p-2 mt-5 min-h-[40px] flex flex-wrap gap-1 items-start text-xs">
                            <template x-for="user in allUsers" :key="user.id">
                                <span
                                    class="bg-gray-100 text-gray-800 px-2 py-1 rounded-md cursor-pointer hover:bg-gray-200"
                    ...
                    <div class="flex gap-4 mb-4">
                        ...
                        <div class="flex-1 min-w-[150px]">
                            <label for="venue" class="block font-medium text-sm text-gray-700">催事場所<span class="text-red-500">*</span></label>
                            <input id="venue" name="venue" type="text" value="{{ old('venue') }}"
                                class="block w-full py-1.5 mt-1 border-gray-300 rounded-md focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm"
                                required>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="description" name="description" rows="4"
                            class="block py-1.5 mt-1 w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm">{{ old('description') }}</textarea>
    ...
    <script>
        ...
        flatpickr("#end_date", { dateFormat: "Y-m-d", allowInput: true, defaultDate: document.getElementById('start_date').value });
    </script>
...
function userChips(allUsers, oldSelectedIds) {
    return {
        ...
        updateDropdownPosition() {
            ...
            const rect = this.$refs.chipContainer.getBoundingClientRect(); // ここをチップ全体に変更
            this.dropdownEl.style.top = (rect.bottom + window.scrollY) + 'px';
            this.dropdownEl.style.left = (rect.left + window.scrollX) + 'px';
            this.dropdownEl.style.width = rect.width + 'px'; // 幅をチップ全体に合わせる
