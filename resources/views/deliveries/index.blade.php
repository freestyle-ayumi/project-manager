        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-2 text-gray-900" style="@media (max-width: 400px) {padding: 0.5rem;}">
                    ...
                    {{-- 検索・フィルターフォーム --}}
                    <form action="{{ route('deliveries.index') }}" method="GET" class="mb-2 p-2 rounded-md shadow-sm bg-white">
                    ...
                    <div class="mb-2 text-right">
                        <a href="{{ route('deliveries.create') }}" class="inline-flex items-center pt-2 pb-1.5 pr-2 pl-1 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                            ＋新規
                        </a>
                    </div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 text-xs text-gray-500" style="height:10px;">
                                ...
                                <tr>
                                    <td class="px-2 py-1 whitespace-nowrap text-center">
                                        <a href="{{ route('deliveries.show', $delivery) }}" class="text-blue-600 hover:text-fuchsia-600 hover:underline">
                                    ...
                                    <td class="px-2 py-1">
                                        ...
                                        @if ($delivery->project)
                                            <a href="{{ route('projects.show', $delivery->project) }}" class="text-blue-600 hover:text-fuchsia-600 ml-1 inline-block align-middle" title="イベント詳細へ">
                                    ...
                                    <td class="px-2 py-1 whitespace-nowrap text-center">
                                        {{ $delivery->client->abbreviation ?? 'N/A' }}
                                    </td>
                                    <td class="px-2 py-1 whitespace-nowrap">
                                        {{ $delivery->subject }}
                                    </td>
                                    <td class="p-1 whitespace-nowrap text-center">
                                        {{ $delivery->delivery_date ? \Carbon\Carbon::parse($delivery->delivery_date)->format('Y/m/d') : '未設定' }}　{{ $delivery->delivery_location ?? '未設定' }}
                                    </td>
                                    <td class="px-2 py-1 whitespace-nowrap text-center">
                                        {{ $delivery->payment_terms ?? '未設定' }}
                                    </td>
                                    <td class="px-2 py-1 sm:pr-3 whitespace-nowrap text-right">
                                        ¥<span class="text-sm">{{ number_format($delivery->total_amount) }}</span>
                                    </td>
                                    <!-- ステータス（見積書と同じ実装） -->
                                    <td class="px-2 py-1 whitespace-nowrap text-center">
                                        <div class="inline-flex items-center justify-center gap-1 text-xs">
                                            <button type="button" 
                                                    class="status-button px-2 py-1 rounded-full cursor-pointer whitespace-nowrap
                                                        {{ $delivery->status === '作成済み' ? 'bg-gray-200 text-gray-800' : '' }}
                                                        {{ $delivery->status === '発行済み' ? 'bg-blue-200 text-blue-800' : '' }}
                                                        {{ $delivery->status === '送信済み' ? 'bg-green-200 text-green-800' : '' }}"
                                                    data-delivery-id="{{ $delivery->id }}"
                                                    data-current-status="{{ $delivery->status }}"
                                                    @if ($delivery->status === '送信済み') disabled @endif>
                                                {{ $delivery->status }}
                                            </button>
                                        </div>
                                    </td>
                                    <td class="px-2 py-1 whitespace-nowrap text-center">
                                        <div class="flex items-center justify-center space-x-1">
                                            <a href="{{ route('deliveries.show', $delivery) }}" class="text-blue-600 hover:text-blue-400" title="詳細">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-box-arrow-right" viewBox="0 0 16 16">
                                                    <path fill-rule="evenodd" d="M10 12.5a.5.5 0 0 1-.5.5h-8a.5.5 0 0 1-.5-.5v-9a.5.5 0 0 1 .5-.5h8a.5.5 0 0 1 .5.5v2a.5.5 0 0 0 1 0v-2A1.5 1.5 0 0 0 9.5 2h-8A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h8a1.5 1.5 0 0 0 1.5-1.5v-2a.5.5 0 0 0-1 0z" />
                                                    <path fill-rule="evenodd" d="M15.854 8.354a.5.5 0 0 0 0-.708l-3-3a.5.5 0 0 0-.708.708L14.293 7.5H5.5a.5.5 0 0 0 0 1h8.793l-2.147 2.146a.5.5 0 0 0 .708.708z" />
                                                </svg>
                                            </a>
                                            <a href="{{ route('deliveries.edit', $delivery) }}" class="text-emerald-600 hover:text-emerald-400" title="編集">
