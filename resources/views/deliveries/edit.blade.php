                    ...
                    <form action="{{ route('deliveries.update', $delivery) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-4 mb-2">
                            ...
                            <div class="md:col-span-2">
                                <label for="delivery_number" class="block text-sm font-medium text-gray-700">見積番号<span class="text-red-500">*</span></label>
                                <input type="text" name="delivery_number" id="delivery_number"
                                        class="block w-full py-1.5 border-gray-300 rounded-md focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm @error('delivery_number') border-red-500 @enderror"
                                        value="{{ old('delivery_number', $delivery->delivery_number) }}" required>
                            ...
                            <div class="md:col-span-6">
                                ...
                                <select name="project_id" id="project_id" class="block w-full py-1.5 border-gray-300 rounded-md focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm @error('project_id') border-red-500 @enderror"
                                    ...
                                    @foreach($projects as $project)
                                        @php
                                            $dateSuffix = $project->start_date
                                                ...
                                                : '';
                                        @endphp
                                        <option value="{{ $project->id }}" data-client-id="{{ $project->client_id }}" {{ old('project_id', $delivery->project_id) == $project->id ? 'selected' : '' }}>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-4 mb-2">
                            ...
                            <div class="md:col-span-8">
                                ...
                                <input type="text" id="subject" name="subject"
                                        class="block w-full py-1.5 border-gray-300 rounded-md focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm @error('subject') border-red-500 @enderror"
                                        value="{{ old('subject', $delivery->subject) }}" required>
                                @error('subject')
                                    <p class="text-red-500 text-xs">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="expiry_date" id="expiry_date" class="block w-full py-1.5 border-gray-300 rounded-md focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm @error('expiry_date') border-red-500 @enderror" value="{{ old('expiry_date', $delivery->expiry_date) }}">
                                @error('expiry_date')
                                    <p class="text-red-500 text-xs">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <div class="relative">
                                    <input type="text" name="issue_date" id="issue_date" class="flatpickr block w-full py-1.5 border-gray-300 rounded-md focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm @error('issue_date') border-red-500 @enderror" value="{{ old('issue_date', $delivery->issue_date) }}" required>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-4 mb-2">
                            ...
                            <div class="md:col-span-12 grid grid-cols-1 md:grid-cols-12 gap-4">
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <div class="relative">
                                        <input type="text" name="delivery_date" id="delivery_date" class="flatpickr block w-full py-1.5 border-gray-300 rounded-md focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm" value="{{ old('delivery_date', $delivery->delivery_date) }}">
                                        <!-- カレンダーアイコン -->
                                        <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-400 mb-1" fill="none"
                                                viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <div class="md:col-span-7">
                                    ...
                                    <input type="text" name="delivery_location" id="delivery_location" class="block w-full py-1.5 border-gray-300 rounded-md focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm" value="{{ old('delivery_location', $delivery->delivery_location) }}" maxlength="255">
                                ...
                                <div class="md:col-span-3">
                                    ...
                                    <input type="text" name="payment_terms" id="payment_terms" class="block w-full py-1.5 border-gray-300 rounded-md focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm" value="{{ old('payment_terms', $delivery->payment_terms) }}" maxlength="255">
                                    @error('payment_terms')
                                        <p class="text-red-500 text-xs">{{ $message }}</p>
                                    @enderror
                                </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-4 mb-2">
                            ...
                            <div class="md:col-span-12">
                                ...
                                <textarea name="notes" id="notes" rows="3" class="block w-full py-1.5 border-gray-300 rounded-md focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm">{{ old('notes', $delivery->notes) }}</textarea>
                                @error('notes')
                                    <p class="text-red-500 text-xs">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div id="items-container" class="space-y-4 border p-2 rounded-md">
                            <div class="space-y-4">
                                @foreach(old('items', $delivery->items) as $index => $item)
                                    @php
                                    ...
                                    <div class="item-row grid grid-cols-1 md:grid-cols-12 gap-4 items-end relative">
                                        ...
                                        <div class="md:col-span-5"> {{-- 品名 --}}
                                            ...
                                            <input type="text" name="items[{{ $index }}][item_name]" id="item_name_{{ $index }}"
                                                    class="block w-full py-1.5 border-gray-300 rounded-md focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm @error('items.' . $index . '.item_name') border-red-500 @enderror"
                                        ...
                                        <div class="md:col-span-1"> {{-- 数量 --}}
                                            ...
                                            <input type="number" min="1" step="1" name="items[{{ $index }}][quantity]" id="quantity_{{ $index }}" class="text-right block w-full sm:pr-0 py-1.5 border-gray-300 rounded-md focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm item-quantity" value="{{ $itemQuantity }}" >
                                        </div>
                                        <div class="md:col-span-1"> {{-- 単位 --}}
                                            <label for="unit_{{ $index }}" class="block text-sm font-medium text-gray-700">単位</label>
                                            <input type="text" name="items[{{ $index }}][unit]" id="unit_{{ $index }}" class="text-right block w-full py-1.5 border-gray-300 rounded-md focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm" value="{{ $itemUnit }}">
                                        </div>
                                        <div class="md:col-span-1"> {{-- 税率 --}}
                                            ...
                                            <input type="number" step="1" name="items[{{ $index }}][tax_rate]" id="tax_rate_{{ $index }}" class="text-right block w-full sm:pr-0 py-1.5 border-gray-300 rounded-md focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm item-tax-rate" value="{{ old('items.' . $index . '.tax_rate', isset($itemTaxRate) ? intval($itemTaxRate) : 0) }}" >
                                        </div>
                                        <div class="md:col-span-2"> {{-- 小計 --}}
                                            <label for="subtotal_{{ $index }}" class="block text-sm font-medium text-gray-700item-subtotal">小計</label>
                                            @php
                                            ...
                                            @endphp
                                            <input type="text" id="subtotal_{{ $index }}" class="block w-full py-1.5 border-gray-300 rounded-md bg-gray-100 text-sm cursor-not-allowed item-subtotal" value="{{ $rowTotal }}" readonly>
                                ...
                                @endforeach
                                {{-- 明細がない場合は初期行を追加 --}}
                                @if($delivery->items->isEmpty() && !old('items'))
                        ...
                        <div class="mx-2 my-4">
                            ...
                            <div class="flex justify-end space-x-2">
                                ...
                                <a href="{{ route('deliveries.index') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold pt-2 pb-1.5 pr-2 pl-2 text-sm rounded-md">
                                    キャンセル
                                </a>
                            </div>
                        </div>
    ...
    <script>
        ...
        <div class="item-row grid grid-cols-1 md:grid-cols-12 gap-4 items-end relative">
            <div class="md:col-span-5">
                ...
                <input type="text" name="items[${itemIndex}][item_name]" id="item_name_${itemIndex}" class="block w-full py-1.5 border-gray-300 rounded-md focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm" required>
            </div>
            <div class="md:col-span-2">
